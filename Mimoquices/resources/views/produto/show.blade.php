                ...
                <details class="accordion accordion-personalizacao-mobile">
                    ...
                    <div class="accordion-content">
                        @if($podePersonalizar && !empty($opcoesDisponiveis))
                            <form method="POST" action="{{ route('produto.personalizar', $produto->url_completo) }}" class="formulario-personalizacao"id="meuFormulario">
                                @csrf
...
<div class="bg-white d-flex justify-content-center py-5">
    <div class="container">
        <form method="POST" action="{{ route('produto.personalizar', $produto->url_completo) }}" class="formulario-personalizacao-desktop"id="meuFormulario">
            @csrf
            ...
            @if ($errors->any())
                <div class="alert-erros-personalizacao" style="margin: 1.5rem auto; width: 100%;">
                    <strong>Erros encontrados:</strong>
                    <ul>
                        @foreach ($errors->all() as $error)
                        <li>{{ $error }}</li>
...
<script>
    ...
    miniaturas.forEach(img => {
        img.addEventListener('click', () => {
            ...
            window.scrollTo({
                top: 0,
                behavior: 'smooth' // faz animação suave
            });

            // Alternativa: scroll até à própria imagem principal
