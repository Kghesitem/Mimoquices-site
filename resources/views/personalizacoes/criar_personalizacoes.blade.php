...
<div class="auth-container auth-container-full">
    ...
    {{-- FORMULÁRIO --}}
    <form method="post" enctype="multipart/form-data" action="{{ route('personalizacao.store') }}" class="auth-form" style="max-width: 100%;">
        @csrf
        @method('post')
        ...
        <div class="form-grid-layout">
            ...
            <div class="col-media">
                ...
                <div class="form-group">
                    ...
                    <select id="categoria" class="form-input form-select" name="tipo_de_input" onchange="verificaCategoria()" required>
                        <option value="texto" {{ old('tipo_de_input') == 'texto' ? 'selected' : '' }}>Em forma de texto</option>
                        <option value="select" {{ old('tipo_de_input') == 'select' ? 'selected' : '' }}>Seleciona apenas uma opção entre várias possíveis</option>
                        <option value="checkbox" {{ old('tipo_de_input') == 'checkbox' ? 'selected' : '' }}>Seleciona uma ou mais opções entre várias possíveis</option>
                    </select>
                </div>
                ...
                {{-- Bloco Dinâmico: Select / Checkbox (Estilizado de forma limpa e integrada) --}}
            ...
            <div class="grid-full-width">
                <button type="submit" class="btn-submit" style="margin-top: 1.5rem; display: flex; align-items: center; justify-content: center; gap: 0.5rem;" onclick="this.disabled=true; this.form.submit();">
                    <x-heroicon-c-bookmark style="width: 1.25rem; height: 1.25rem;"/> Guardar Personalização
                </button>
            </div>
    ...
    <div class="auth-footer" style="margin-top: 3rem;">
        <p>Deseja associar estas personalizações a um produto existente?</p>
        <a href="{{ url('/dashboard') }}">Voltar para o Catálogo →</a>
    </div>
</div>
</main>
...
<script>
    ...
    // Inicializa o estado correto com base na seleção (útil para o redirecionamento com 'old')
    document.addEventListener("DOMContentLoaded", function() {
        verificaCategoria();
    });
