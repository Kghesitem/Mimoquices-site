...
<div class="card mb-4 border-0 shadow-sm" style="border-radius: 1.5rem; max-width: 1200px; margin: 0 auto 2rem;">
    <div class="card-body p-3 d-flex flex-wrap gap-3 align-items-center" style="background-color: white; border-radius: 1.5rem; border: 1px solid var(--color-border);">
        ...
        <select id="filtroTipos" class="form-select w-auto" style="border-radius: 1rem; border-color: var(--color-border); appearance: none; -webkit-appearance: none; -moz-appearance: none; background-image: none; padding-right: 1rem;">">
        ...
        <select id="ordenar" class="form-select w-auto" style="border-radius: 1rem; border-color: var(--color-border); appearance: none; -webkit-appearance: none; -moz-appearance: none; background-image: none; padding-right: 1rem;">
    ...
    <div class="table-responsive">
        <table class="table table-hover m-0" style="font-family: inherit;">
            ...
            <tbody>
                ...
                <tr class="align-middle item-produto" data-tipo="{{ $produto->tipo_prod }}" style="border-bottom: 1px solid var(--color-border);">
...
<div id="cardView" class="container-fluid px-0" style="display: flex; flex-wrap: wrap; gap: 15px; justify-content: center;">
    ...
    @endif
    @foreach ($produtos as $produto)
        <div class="dash-card item-produto" data-tipo="{{ $produto->tipo_prod }}" style="flex: 0 1 280px; min-width: 250px; background-color: white; border: 1px solid var(--color-border); display: flex; flex-direction: column; padding: 1.5rem; border-radius: 1.5rem;">
...
$(document).ready(function() {
    ...
    function filtrarProdutos() {
        ...
        // Seleciona todos os itens (tanto linhas da tabela quanto cards)
        const itens = document.querySelectorAll('.item-produto');

        itens.forEach(item => {
            // Busca o título dentro do item (ajuste o seletor conforme seu HTML)
        ...
        const msgVazio = document.querySelectorAll('.text-center.py-4, .text-center.py-5');
        msgVazio.forEach(msg => {
            if(msg.textContent.includes("Nenhum produto")) {
                msg.closest('tr, div').style.display = encontrouAlgo ? 'none' : 'block';
            }
        });
    }

    // Função de Ordenação (apenas para os Cards para simplificar)
    function ordenarProdutos() {
        const ord = ordenar.value;
        const containerCards = document.getElementById('cardView');
        const cards = Array.from(containerCards.querySelectorAll('.dash-card'));

        cards.sort((a, b) => {
            const titleA = a.querySelector('h3').textContent.trim().toLowerCase();
            const titleB = b.querySelector('h3').textContent.trim().toLowerCase();
            
            if (ord === 'nome_asc') return titleA.localeCompare(titleB);
