...
public function personalizarProduto(Request $request, $url_completo)
{
    ...
    $pedido = Pedido::create([
        ...
        'estado' => 'pendente', // assumindo que queres um estado inicial
    ]);

    // 3️⃣ Campos de personalização
    $campos = [
        'texto_capa',
        'formato_agenda',
        'acessorio',
    ...
    // 4️⃣ Validar os campos
    $request->validate([
        'texto_capa' => ['required', 'string', 'max:30'],
        'formato_agenda' => ['required'],
        'cor_argolas' => ['required'],
    ]);

    // 5️⃣ Criar personalizações
    $personalizacoes = []; // vamos guardar para enviar no email
